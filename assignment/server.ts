...
const cfg = {
  ...
  rtspUrl: process.env.RTSP_URL ?? "rtsp://viewer:CHANGE_ME_VIEW@mediamtx:8554/cam1",
  grabTimeoutMs: Number(process.env.GRAB_TIMEOUT_MS ?? 20000),

  // Background job defaults (overridable per POST /start).
  intervalMs: Number(process.env.INTERVAL_MS ?? 1000),   // pause between captures
...
const idleJob = (): Job => ({
  running: false, started_at: null, finished_at: null, reason: null,
  interval_ms: cfg.intervalMs, max_captures: cfg.maxCaptures, note: "",
  consecutive_failures: 0,
});

const emptyAssignment = (): Assignment => ({
  title: "", subject: "", instructions_latex: "", problems: [],
});
...
function loadState(): State {
  try {
    if (existsSync(STATE_FILE)) {
      const loaded = JSON.parse(readFileSync(STATE_FILE, "utf8")) as State;
...
function saveState() {
  ...
  writeAtomic(ASSIGNMENT_FILE, JSON.stringify({
...
async function grabViaGateway(): Promise<Buffer> {
  ...
  try {
    res = await fetch(url, {
      headers: cfg.snapshotToken ? { "x-snapshot-token": cfg.snapshotToken } : {},
    ...
    throw new Error(`snapshot API unreachable at ${url.origin}${url.pathname}: ${e?.message ?? e}`);
  }
  if (!res.ok) {
    throw new Error(`snapshot API ${res.status}: ${(await res.text()).slice(0, 300)}`);
  }
  const jpeg = Buffer.from(await res.arrayBuffer());
  if (jpeg.length === 0) throw new Error("snapshot API returned an empty image");
  return jpeg;
}
...
async function grabViaRtsp(): Promise<Buffer> {
  ...
  try {
    ...
    if (jpeg.length === 0) {
      throw new Error(`ffmpeg produced no frame — is the stream publishing? ${err.trim()}`);
    }
    return jpeg;
  } finally {
    clearTimeout(timer);
...
const RESPONSE_SCHEMA = {
  type: "object",
  properties: {
    frame_quality: { type: "string", enum: ["good", "blurry", "dark", "glare", "no_paper"] },
    framing: {
      type: "object",
      properties: {
        full_page_visible: { type: "boolean" },
        cut_off_edges: { type: "array", items: { type: "string", enum: ["top", "bottom", "left", "right"] } },
        camera_advice: {
          type: "string",
          enum: ["ok", "move_up", "move_down", "move_left", "move_right",
                 "move_closer", "move_farther", "refocus", "reduce_glare", "reposition_paper"],
        },
        advice_detail: { type: "string" },
      },
      required: ["full_page_visible", "cut_off_edges", "camera_advice", "advice_detail"],
    },
    assignment: {
      type: "object",
      properties: {
  ...
  required: ["frame_quality", "framing", "assignment", "changes", "confidence", "done"],
};
...
type GeminiResult = {
  frame_quality: string;
  framing: { full_page_visible: boolean; cut_off_edges: string[]; camera_advice: string; advice_detail: string };
  assignment: Assignment;
  changes: string[];
  confidence: number;
  done: boolean;
...
async function readFrame(jpeg: Buffer, note: string): Promise<GeminiResult> {
  ...
  const url = `${cfg.geminiBase}/v1beta/models/${cfg.model}:generateContent`;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Gemini ${res.status} (model="${cfg.model}"): ${raw.slice(0, 800)}`);
  }
  let body: any;
  try { body = JSON.parse(raw); } catch { throw new Error(`Gemini returned non-JSON: ${raw.slice(0, 400)}`); }
  const text = body?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!text) throw new Error(`Gemini returned no content: ${raw.slice(0, 400)}`);
  try {
    return JSON.parse(text) as GeminiResult;
  } catch {
    throw new Error(`model output was not valid JSON: ${String(text).slice(0, 400)}`);
  }
}
...
function eventStream(): Response {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(c) {
      ...
      c.enqueue(frame("snapshot", { at: new Date().toISOString(), ...snapshot() }));
    },
...
async function doCapture(note: string) {
  if (capturing) throw Object.assign(new Error("a capture is already in progress"), { status: 409 });
  capturing = true;
  const startedAt = Date.now();
  const startedVersion = state.version;
  try {
    emit("capture_started", { n: state.capture_count + 1, note: note || null });
    ...
    if (state.version !== startedVersion) {
      emit("capture_discarded", { reason: "assignment was reset during this capture" });
      throw Object.assign(new Error("assignment was reset during this capture"), { status: 409 });
    }
    ...
    const needs_adjustment =
      !result.framing.full_page_visible || result.framing.camera_advice !== "ok";
    ...
    if (state.done) emit("done", { version: state.version, problems: state.assignment.problems?.length ?? 0 });
...
function finishJob(reason: string) {
  ...
  console.log(`[job] finished: ${reason} after ${state.capture_count} capture(s)`);
}
...
async function jobLoop() {
  ...
  try {
    while (state.job.running) {
      ...
      if (state.capture_count >= state.job.max_captures) return finishJob("max_captures");
      ...
      try {
        ...
        console.error(`[job] capture failed (${state.job.consecutive_failures}/${cfg.maxFailures}):`, e?.message ?? e);
        if (state.job.consecutive_failures >= cfg.maxFailures) return finishJob("failed");
      }
      saveState();
...
function startJob(opts: { note?: string; interval_ms?: number; max_captures?: number }) {
  state.job = {
    running: true,
...
function toMarkdown(a: Assignment): string {
  ...
  for (const p of a.problems ?? []) {
    lines.push(`## ${p.number}${p.complete ? "" : "  _(incomplete)_"}`, "", p.statement_latex, "");
  }
  return lines.join("\n");
}
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (path === "/health") {
      return json({
        ok: true, version: state.version, captures: state.capture_count,
        done: state.done, job_running: state.job.running,
      });
    }
    ...
    if (path === "/start" && req.method === "POST") {
      if (state.job.running) {
        return json({ ok: false, error: "a job is already running", job: state.job }, 409);
      }
      if (state.done) {
      ...
      let opts: any = {};
      try { opts = (await req.json()) ?? {}; } catch { /* empty body is normal */ }
      const job = startJob({
        note: typeof opts.note === "string" ? opts.note : undefined,
        interval_ms: Number.isFinite(opts.interval_ms) ? Number(opts.interval_ms) : undefined,
        max_captures: Number.isFinite(opts.max_captures) ? Number(opts.max_captures) : undefined,
      });
      // 202: accepted and running in the background — watch /events for progress.
      return json({ ok: true, started: true, job, watch: "/events" }, 202);
    }
    ...
    if (path === "/frame.jpg" && req.method === "GET") {
      if (!existsSync(FRAME_FILE)) return json({ error: "no frame captured yet" }, 404);
      return new Response(readFileSync(FRAME_FILE), { headers: { "content-type": "image/jpeg" } });
    }
    ...
    if (path === "/reset" && req.method === "POST") {
      ...
      if (previous.job.running) {       // a reset implies "stop what you're doing"
        previous.job.running = false;
        wakeFromSleep?.();
      }
      if (previous.capture_count > 0) {
        writeAtomic(
          join(ARCHIVE_DIR, `v${previous.version}-${previous.created_at.replace(/[:.]/g, "-")}.json`),
          JSON.stringify(previous, null, 2),
        );
      }
      state = newState(previous.version + 1);
      saveState();
      emit("reset", { version: state.version, archived: previous.capture_count > 0 });
      return json({ ok: true, version: state.version, archived: previous.capture_count > 0 });
    }

    return json({
      error: "not found",
      endpoints: ["POST /start", "POST /stop", "GET /events", "POST /capture",
                  "GET /assignment", "GET /assignment.md", "GET /state",
                  "GET /frame.jpg", "POST /reset", "GET /health"],
    }, 404);
  },
});
...
console.log(`  frames: ${cfg.snapshotUrl
  ? `${cfg.snapshotUrl} (gateway snapshot API)`
  : `${cfg.rtspUrl.replace(/\/\/[^@]*@/, "//***@")} (direct RTSP)`}`);
console.log(`  data:   ${cfg.dataDir}  (version ${state.version}, ${state.capture_count} captures)`);
if (!cfg.geminiKey) console.warn("  [!] GEMINI_API_KEY is not set — captures will fail");
if (!cfg.token) console.warn("  [!] API_TOKEN is empty — this server is unauthenticated");
...
if (state.job.running) {
  if (cfg.autoResume && !state.done && state.capture_count < state.job.max_captures) {
    console.log(`[job] resuming interrupted job (${state.capture_count}/${state.job.max_captures} captures used)`);
    emit("job_resumed", { from_capture: state.capture_count, max_captures: state.job.max_captures });
    void jobLoop();
  } else {
