...
Open the companion app → **Photo** tab → *Phone gallery bridge* → paste. It
saves as you type — there is no Save button — and the token in that URL survives
restarts (`gallery.py` keeps it in `~/.evens-gallery-token`), so this is a
one-time paste. Edit the field to change it, empty it to forget it, and use
**Check** to re-test once Termux is up.

The URL is stored on the **document server** (`/settings/gallery-bridge`), not
in the browser. It used to be in localStorage, which the WebView does not keep
across launches — so the app asked for the URL again every time it reopened.
Storing it server-side also makes the next line true rather than aspirational:
