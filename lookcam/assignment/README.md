...
also accepted only when the model marks them `clear: true` with confidence at
least `MIN_PROBLEM_CONFIDENCE` (default `0.85`). The whole frame must have
...
| `model_response` | model replied | `camera_advice`, `advice_detail`, `next_target`, `unreadable`, `region`, `edges_seen`, `more_content_beyond`, `changes`, `confidence`, `done`, `ms` |
